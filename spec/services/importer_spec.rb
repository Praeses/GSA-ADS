...
require './src/services/importer.rb'

describe "Services::Importer" do


	it "should be defined" do
		Services::Importer.should_not be nil
	end


	it "should be able to pull down a batch of data" do
		subject = Services::Importer.new
	...
	it "should should be save the page of data to disk" do
		subject = Services::Importer.new
		#subject.pull_drug_event_page(1)
		original_size = `wc -l #{Services::Importer::DRUG_DATA_CACHE}`.to_i
		subject.pull_drug_event_page(1)
		subject.save_pulled_drug_events
		new_size = `wc -l #{Services::Importer::DRUG_DATA_CACHE}`.to_i
	...
	it "should be able to pull down data for the first day" do
		subject = Services::Importer.new
		subject.pull_drug_event_page(1, Services::Importer::FIRST_DAY)
		subject.drug_events.size.should_not be 0
	end
